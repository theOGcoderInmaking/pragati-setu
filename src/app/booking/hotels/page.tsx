...
const RECOMMENDED_HOTEL = {
    name: "Park Hyatt Tokyo",
    stars: 5,
    location: "SHINJUKU, TOKYO",
    guideVerified: "Dec 2025",
    dimensions: [
        { label: "Cleanliness", value: 9.4 },
        { label: "Noise", value: 8.1 },
        { label: "Safety", value: 9.7 },
        { label: "Staff", value: 9.6 },
        { label: "Value", value: 8.2 },
        { label: "Location", value: 9.8 },
        { label: "Breakfast", value: 8.8 },
        { label: "Design", value: 9.5 },
    ],
    why: "Quietest top-floor rooms despite central location. Female solo traveler rating: 9.8. 24hr security. Exceptional concierge.",
    price: "₹12,400",
};
                    ...
                    <div className={styles.resultsLayout}>
                        ...
                        <main className={styles.resultsMain}>
                            ...
                            <div className={styles.hotelCardRecommended}>
                                <span className={styles.recommendedBadge}>🧭 PRAGATI SETU RECOMMENDS</span>
                                <div className={styles.hotelName}>{RECOMMENDED_HOTEL.name}</div>
                                <div className={styles.hotelLocation}>{RECOMMENDED_HOTEL.location}</div>
                                <div className={styles.hotelMeta}>
                                    <span className={styles.stars}>{"★".repeat(RECOMMENDED_HOTEL.stars)}</span>
                                </div>
                                <div className={styles.guideVerified}>✓ Guide Verified — {RECOMMENDED_HOTEL.guideVerified}</div>
                                <div className={styles.dimensionScores}>
                                    {RECOMMENDED_HOTEL.dimensions.map(d => (
                                        <div key={d.label} className={styles.dimensionItem}>
                                            <span className={styles.dimensionLabel}>{d.label}</span>
                                            <div className={styles.dimensionBar}>
                                                <div className={styles.dimensionFill} style={{ width: `${(d.value / 10) * 100}%` }} />
                                            </div>
                                            <span className={styles.dimensionValue}>{d.value}</span>
                                        </div>
                                    ))}
                                </div>
                                <div className={styles.whyBox}>
                                    <span className={styles.whyLabel}>Why we recommend this</span>
                                    <p className={styles.whyText}>&ldquo;{RECOMMENDED_HOTEL.why}&rdquo;</p>
                                </div>
                                <div className={styles.priceRow}>
                                    <div>
                                        <div className={styles.price}>{RECOMMENDED_HOTEL.price}</div>
